...
- **네이버 증권 JSON API**를 1순위로 사용하고, 실패하면 예전 HTML 페이지에 **6개의 독립 파서**를 순서대로 시도합니다
- EUC-KR 인코딩 정확 처리
...
- 대화에 밀려 올라갔으면 `/yen-board` 로 맨 아래에 다시 소환
...
| 변수                      | 기본값           | 설명                                             |
| ------------------------- | ---------------- | ------------------------------------------------ |
| `NAVER_JPY_API_URL`       | 네이버 증권 API  | 1순위 수집 경로(JSON). `off` 면 HTML 파서만 사용 |
| `NAVER_JPY_URL`           | 네이버 엔화 상세 | API 실패 시 HTML 폴백. 보통 기본값 그대로 사용   |
...
| `/yen-alert remove`       | 알림 삭제 (ID 지정)                                              | **본인만** |
| `/yen-board`              | 자동 갱신 보드를 채널 맨 아래에 다시 띄움 (30초 쿨다운)          | **본인만** |
...
/yen-alert list         → 등록된 알림 + 삭제 드롭다운
/yen-alert remove id:3  → ID 를 아는 경우 바로 삭제
```

**삭제는 드롭다운이 편합니다.** `/yen-alert list` 를 실행하면 목록 아래에 선택 메뉴가 붙습니다.
...
- 여러 개를 한 번에 선택해 삭제할 수 있습니다
- 삭제하면 목록이 즉시 갱신됩니다
- 지울 수 있는 알림만 목록에 나옵니다 (골랐는데 거부되는 일이 없습니다)
- 본인이 연 목록에서만 동작합니다
...
4. 메시지를 실수로 지웠다면 — 다음 수집(1분 이내)에 자동으로 새로 만듭니다.
5. 대화에 밀려 위로 올라갔다면 — Discord 에서 **`/yen-board`** 를 실행하면 맨 아래에 다시 띄웁니다. (재시작 불필요)
